...
st.title("Docking System App v0.3")
...
if report_choice == "Individual Performance":
    ...
    # Workload summary should count all records (including assignments)
    ...
    # -----------------------------------------------------------
    # Visualization: Workload Summary Comparison
    # -----------------------------------------------------------
    import matplotlib.pyplot as plt
    from io import BytesIO

    # Prepare data for visualization using the computed variables
    categories = [driver, "Median", "Average"]
    load_counts = [load_count, median_load_count, avg_load_count]
    durations = [total_duration, median_total_duration, avg_total_duration]

    # Custom color as specified
    blue_color = "#086ccc"

    # Create subplots for the two bar charts
    fig, ax = plt.subplots(1, 2, figsize=(11, 4), dpi=80)

    # Load Count Comparison
    ax[0].bar(categories, load_counts, color=[blue_color, 'gray', 'gray'])
    ax[0].set_ylabel("Load Count")
    ax[0].set_title("Load Count Comparison")

    # Total Duration Comparison
    ax[1].bar(categories, durations, color=[blue_color, 'gray', 'gray'])
    ax[1].set_ylabel("Total Duration (min)")
    ax[1].set_title("Total Duration Comparison")

    # Adjust layout so that everything fits nicely
    plt.tight_layout()

    # Convert the Matplotlib figure to a PNG image and display it with a max width of 1100px
    buf = BytesIO()
    ...
    def collaborator_analysis(df, selected_driver, valid_drivers, min_collabs=5):
        ...
        # Determine best collaborators (lowest average diff) and poor collaborators (highest average diff)
        best_collab = collab_df.sort_values("Average Diff (min)").head(5)
        worst_collab = collab_df.sort_values("Average Diff (min)", ascending=False).head(5)
        return best_collab, worst_collab

    # Call the collaborator analysis function using the date-filtered data (df from earlier)
    best_collab, worst_collab = collaborator_analysis(date_filtered_df, driver, valid_drivers, min_collabs=5)

    # Build HTML for the collaborator analysis section using similar styling and a wrapper
    collab_html = "<div class='modules-container'>"
    collab_html += "<h2>Collaborator Analysis for {}</h2>".format(driver)
    # Add an explanation for the Average Diff (min) metric
    collab_html += """
    <div class='explanation'>
    ...
    # Display the collaborator analysis section
...
# -----------------------------------------------------------
    ...
    # First inject the CSS globally
    st.markdown(css, unsafe_allow_html=True)
    
    # Then create a scrollable container with proper HTML rendering
    components.html(
        f"""
